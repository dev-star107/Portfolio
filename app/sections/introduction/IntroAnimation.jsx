...
export function WelcomeAnimation() {
	...
	const glareOpacity = useTransform(
	  [x, y],
	  ([dx, dy]) => Math.sqrt(dx ** 2 + dy ** 2) / (maxTilt * Math.sqrt(2)) * glareMaxOpacity
	);
	...
	useEffect(() => {
	  const handleMouseMove = (event) => {
		...
		x.set((clientX - centerX) / (rect.width / 2) * maxTilt);
		y.set((clientY - centerY) / (rect.height / 2) * maxTilt);
	  };
		...
		>
			<motion.div 
				...
				style={{
					...
					transition: `transform ${speed / 1000}s cubic-bezier(.03,.98,.52,.99)`,
				  }}
			>
              <img oncontextmenu="return false;" loading="lazy" className="intro-img" src="/assets/img/me.jpg" alt="intro-image"/>
				{/* <div className="js-tilt-glare absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none" >
					<div className="js-tilt-glare-inner absolute top-1/2 left-1/2 pointer-events-none bg-gradient-to-b from-transparent to-white rotate-180 origin-top-left -translate-x-1/2 -translate-y-1/2 opacity-0 w-[886px] h-[886px]"></div>
				</div> */}
			</motion.div>
			{/* <svg
